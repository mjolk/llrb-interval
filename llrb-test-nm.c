...
struct range_tree { struct node *rbh_root; };
struct range_tree head;
...
static inline void range_tree_LLRB_ROTL(struct node **pivot) {
    ...
    if (((a)->entry.rbe_right = (b)->entry.rbe_left)) ((a)->entry.rbe_right)->entry.rbe_parent = a;
    (b)->entry.rbe_left = a;
    (b)->entry.rbe_color = (a)->entry.rbe_color;
    (a)->entry.rbe_color = 1;
    (b)->entry.rbe_parent = (a)->entry.rbe_parent;
    (a)->entry.rbe_parent = b; *pivot = b; 
}
static inline void range_tree_LLRB_ROTR(struct node **pivot) {
    struct node *b = *pivot; struct node *a = (b)->entry.rbe_left;
    if (((b)->entry.rbe_left = (a)->entry.rbe_right)) ((b)->entry.rbe_left)->entry.rbe_parent = b;
    (a)->entry.rbe_right = b;
    (a)->entry.rbe_color = (b)->entry.rbe_color;
    (b)->entry.rbe_color = 1; (a)->entry.rbe_parent = (b)->entry.rbe_parent;
    (b)->entry.rbe_parent = a; *pivot = a;
}
static inline void range_tree_LLRB_FLIP(struct node *root) {
    (root)->entry.rbe_color = !(root)->entry.rbe_color;
    ((root)->entry.rbe_left)->entry.rbe_color = !((root)->entry.rbe_left)->entry.rbe_color;
    ((root)->entry.rbe_right)->entry.rbe_color = !((root)->entry.rbe_right)->entry.rbe_color;
}
static inline void range_tree_LLRB_FIXUP(struct node **root) {
    if ((((*root)->entry.rbe_right) && (((*root)->entry.rbe_right))->entry.rbe_color == 1) && !(((*root)->entry.rbe_left) && (((*root)->entry.rbe_left))->entry.rbe_color == 1)) range_tree_LLRB_ROTL(root);
    if ((((*root)->entry.rbe_left) && (((*root)->entry.rbe_left))->entry.rbe_color == 1) && ((((*root)->entry.rbe_left)->entry.rbe_left) && ((((*root)->entry.rbe_left)->entry.rbe_left))->entry.rbe_color == 1)) range_tree_LLRB_ROTR(root);
    if ((((*root)->entry.rbe_left) && (((*root)->entry.rbe_left))->entry.rbe_color == 1) && (((*root)->entry.rbe_right) && (((*root)->entry.rbe_right))->entry.rbe_color == 1)) range_tree_LLRB_FLIP(*root);
}
struct node *range_tree_LLRB_INSERT(struct range_tree *head, struct node *elm) {
    struct node **root = &(head)->rbh_root; struct node *parent = 0;
    while (*root) {
        int comp = (intcmp)((elm), (*root));
        parent = *root;
        if (comp < 0) root = &(*root)->entry.rbe_left;
        else if (comp > 0) root = &(*root)->entry.rbe_right;
        else return *root;
    }
    ...
    *root = (elm);
    while (parent && ((((parent)->entry.rbe_left) && (((parent)->entry.rbe_left))->entry.rbe_color == 1) || (((parent)->entry.rbe_right) && (((parent)->entry.rbe_right))->entry.rbe_color == 1))) {
        root = (((parent) == (head)->rbh_root)? &(head)->rbh_root : ((parent) == (((parent))->entry.rbe_parent)->entry.rbe_left)? &(((parent))->entry.rbe_parent)->entry.rbe_left : &(((parent))->entry.rbe_parent)->entry.rbe_right);
        parent = (parent)->entry.rbe_parent;
        range_tree_LLRB_FIXUP(root);
    }
    ((head)->rbh_root)->entry.rbe_color = 0;
    struct node *p = 0; p = elm;
    while(p){
        char left_max[4 +1] = "";
        char right_max[4 +1] = "";
        if((p)->entry.rbe_left){
            strncpy(left_max, (strncmp(((p)->entry.rbe_left)->end_key, ((p)->entry.rbe_left)->max, 4)>0?((p)->entry.rbe_left)->end_key:((p)->entry.rbe_left)->max), 4);
        }
        if((p)->entry.rbe_right){
            strncpy(right_max, (strncmp(((p)->entry.rbe_right)->end_key, ((p)->entry.rbe_right)->max, 4)>0?((p)->entry.rbe_right)->end_key:((p)->entry.rbe_right)->max), 4);
        }
        char maxc[4 +1] = "";
        char max[4 +1] = "";
        strncpy(maxc, (strncmp(left_max, right_max, 4) > 0?left_max:right_max), 4 +1);
        strncpy(max, (strncmp(maxc , (p)->end_key, 4) > 0?maxc:(p)->end_key), 4 +1);
        strncpy((p)->max, (strncmp((p)->max, max, 4)>0?(p)->max:max), 4 +1); p = (p)->entry.rbe_parent;
    }
    return 0;
}
static inline void range_tree_LLRB_MOVL(struct node **pivot) {
    range_tree_LLRB_FLIP(*pivot);
    if (((((*pivot)->entry.rbe_right)->entry.rbe_left) && ((((*pivot)->entry.rbe_right)->entry.rbe_left))->entry.rbe_color == 1)) {
        range_tree_LLRB_ROTR(&(*pivot)->entry.rbe_right);
        range_tree_LLRB_ROTL(pivot); range_tree_LLRB_FLIP(*pivot);
    }
}
static inline void range_tree_LLRB_MOVR(struct node **pivot) {
    range_tree_LLRB_FLIP(*pivot);
    if (((((*pivot)->entry.rbe_left)->entry.rbe_left) && ((((*pivot)->entry.rbe_left)->entry.rbe_left))->entry.rbe_color == 1)) {
        range_tree_LLRB_ROTR(pivot);
        range_tree_LLRB_FLIP(*pivot);
    }
}
static inline struct node *range_tree_DELETEMIN(struct range_tree *head, struct node **root) {
    struct node **pivot = root, *deleted, *parent;
    while ((*pivot)->entry.rbe_left) {
        if (!(((*pivot)->entry.rbe_left) && (((*pivot)->entry.rbe_left))->entry.rbe_color == 1) && !((((*pivot)->entry.rbe_left)->entry.rbe_left) && ((((*pivot)->entry.rbe_left)->entry.rbe_left))->entry.rbe_color == 1)) range_tree_LLRB_MOVL(pivot);
        pivot = &(*pivot)->entry.rbe_left;
    }
    deleted = *pivot; parent = (*pivot)->entry.rbe_parent;
    *pivot = 0;
    while (root != pivot) {
        pivot = (((parent) == (head)->rbh_root)? &(head)->rbh_root : ((parent) == (((parent))->entry.rbe_parent)->entry.rbe_left)? &(((parent))->entry.rbe_parent)->entry.rbe_left : &(((parent))->entry.rbe_parent)->entry.rbe_right);
        parent = (parent)->entry.rbe_parent; range_tree_LLRB_FIXUP(pivot);
    }
    return deleted;
}
struct node *range_tree_LLRB_DELETE(struct range_tree *head, struct node *elm) {
    ...
    while (*root) {
        ...
        if (comp < 0) {
            if ((*root)->entry.rbe_left && !(((*root)->entry.rbe_left) && (((*root)->entry.rbe_left))->entry.rbe_color == 1) && !((((*root)->entry.rbe_left)->entry.rbe_left) && ((((*root)->entry.rbe_left)->entry.rbe_left))->entry.rbe_color == 1)) range_tree_LLRB_MOVL(root);
            root = &(*root)->entry.rbe_left;
        } else {
            if ((((*root)->entry.rbe_left) && (((*root)->entry.rbe_left))->entry.rbe_color == 1)) {
                range_tree_LLRB_ROTR(root);
                comp = (intcmp)(elm, *root);
            } if (!comp && !(*root)->entry.rbe_right) {
                deleted = *root; *root = 0;
                break;
            } if ((*root)->entry.rbe_right && !(((*root)->entry.rbe_right) && (((*root)->entry.rbe_right))->entry.rbe_color == 1) && !((((*root)->entry.rbe_right)->entry.rbe_left) && ((((*root)->entry.rbe_right)->entry.rbe_left))->entry.rbe_color == 1)) {
                range_tree_LLRB_MOVR(root);
                comp = (intcmp)(elm, *root);
            } if (!comp) {
                struct node *orphan = range_tree_DELETEMIN(head, &(*root)->entry.rbe_right);
                (orphan)->entry.rbe_color = (*root)->entry.rbe_color;
                (orphan)->entry.rbe_parent = (*root)->entry.rbe_parent;
                if (((orphan)->entry.rbe_right = (*root)->entry.rbe_right)) ((orphan)->entry.rbe_right)->entry.rbe_parent = orphan;
                if (((orphan)->entry.rbe_left = (*root)->entry.rbe_left)) ((orphan)->entry.rbe_left)->entry.rbe_parent = orphan;
                deleted = *root;
                *root = orphan;
                parent = *root;
                break;
            } else root = &(*root)->entry.rbe_right;
        }
    }
    while (parent) {
        root = (((parent) == (head)->rbh_root)? &(head)->rbh_root : ((parent) == (((parent))->entry.rbe_parent)->entry.rbe_left)? &(((parent))->entry.rbe_parent)->entry.rbe_left : &(((parent))->entry.rbe_parent)->entry.rbe_right);
...
struct node *range_tree_LLRB_FIND(struct range_tree *head, struct node *key) {
    ...
    while (elm) {
        ...
        if (comp < 0) elm = (elm)->entry.rbe_left;
        else if (comp > 0) elm = (elm)->entry.rbe_right;
        else return elm;
    } return 0;
}
...
struct node *range_tree_LLRB_NEXT(struct node *elm) {
    if ((elm)->entry.rbe_right) {
        ...
        if (elm == ((elm)->entry.rbe_parent)->entry.rbe_left) return (elm)->entry.rbe_parent;
        while ((elm)->entry.rbe_parent && elm == ((elm)->entry.rbe_parent)->entry.rbe_right) elm = (elm)->entry.rbe_parent;
...
struct node *range_tree_LLRB_PREV(struct node *elm) {
    if ((elm)->entry.rbe_left) {
        ...
        if (elm == ((elm)->entry.rbe_parent)->entry.rbe_right) return (elm)->entry.rbe_parent;
        while ((elm)->entry.rbe_parent && elm == ((elm)->entry.rbe_parent)->entry.rbe_left) elm = (elm)->entry.rbe_parent;
        return (elm)->entry.rbe_parent;
    } else return 0;
}
typedef void (*merger)(struct node*, struct range_group*);
void range_tree_LLRB_RANGE_MATCHER(struct node *s, struct node *elm, struct range_group *sll, merger merge) {
    if((s)->entry.rbe_left){
    ...
    if((s)->entry.rbe_right) {
        if((strncmp((elm)->start_key, ((s)->entry.rbe_right)->max, 4) <= 0) && (strncmp((elm)->end_key, ((s)->entry.rbe_right)->start_key, 4) >= 0)) {
            range_tree_LLRB_RANGE_MATCHER((s)->entry.rbe_right, elm, sll, merge);
        }
    }
    if((strncmp((elm)->start_key, (s)->end_key, 4) <= 0) && (strncmp((elm)->end_key, (s)->start_key, 4) >= 0)) {
        merge(s, sll);
    }
}
int range_tree_LLRB_RANGE_OVERLAPS(struct range_tree *head, struct node *elm) {
    if((head)->rbh_root == 0) return 0;
    if((strncmp((elm)->start_key, ((head)->rbh_root)->max, 4) <= 0) && (strncmp((elm)->end_key, (range_tree_LLRB_MIN(((head))->rbh_root))->start_key, 4) >= 0)) {
        return 1;
    } return 0;
}
int range_tree_LLRB_RANGE_GROUP_ADD(struct range_tree *head, struct node *elm, struct range_group *sll, merger merge) {
    ...
    if(!(((sll)->slh_first)->next.sle_next) && ((strncmp(((sll)->slh_first)->start_key, (elm)->start_key, 4) <= 0) && (strncmp(((sll)->slh_first)->end_key, (elm)->end_key, 4) >= 0))) {
        return 0;
    }
    return 1;
}
