...
static inline int cmp32(const char *a, const char *b) {
    ...
    int diff_idx = __builtin_ctz(~mask);
    return (unsigned char)a[diff_idx] - (unsigned char)b[diff_idx];
}
...
int intcmp(struct node *e1, struct node *e2) {
  return cmp32(e1->start_key, e2->start_key);
}
LLRB_HEAD(range_tree, node);
struct range_tree head;
LLRB_PROTOTYPE(range_tree, node, entry, intcmp);
LLRB_RANGE_GROUP_GEN(range_tree, node, entry, cmp32, range_group, next)
...
void merge(struct range_tree *t, struct node *to_merge,
  ...
  while (nxt) {
    ...
    if ((cmp32(to_merge->start_key, c->end_key) <= 0) &&
        (cmp32(to_merge->end_key, c->start_key) >= 0)) {
      /**overlap**/
      if (cmp32(to_merge->start_key, c->start_key) > 0) {
        memcpy(to_merge->start_key, c->start_key, KEY_SIZE);
      }
      if (cmp32(to_merge->end_key, c->end_key) < 0) {
        memcpy(to_merge->end_key, c->end_key, KEY_SIZE);
      }
...
int add_to_range(char *start, char *end) {
  struct node nn;
  memset(nn.start_key, 0, KEY_SIZE);
  memset(nn.end_key, 0, KEY_SIZE);
...
int main(void) {
  ...
  for (i = 0; i < sizeof(testdata) / sizeof(testdata[0]); i++) {
    ...
    memset(n->start_key, 0, KEY_SIZE);
    memset(n->end_key, 0, KEY_SIZE);
  ...
  memset(in->start_key, 0, KEY_SIZE);
  memset(in->end_key, 0, KEY_SIZE);
